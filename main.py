...
def update_graph():
    ...
    def animate_ads_graph():
        # this will be updated soon to accomadate ADS animation
        pass
    def animate_graph():
    ...
    if any_change:
        ...
        for node in agent.graph.keys():
            ...
            for child_node ,weight in children.items():
                ...
                if (node, child_node) in visited or (child_node, node) in visited: # to avoid pairs like ('a','b') and ('b','a')
                    continue
...
def weightsUpdate():
    ...
    if validated:
        result = int(document["weights-input"].value)
...
def heuristicsUpdate():
    ...
    if validated:
        ...
        agent.graph[node_heur].heuristics = int(result)
        any_change = True
...
def tool_select(do):
    ...
    if do == "nodeAdd":
        ...
        document["paragh"].innerText = "Select node as "+ document['setstart'].innerText[4:] +" Node"
    elif do == "setgoal":
        document["paragh"].innerText = "Select node as "+ document['setgoal'].innerText[4:] +" Node"    
    
    tool = do

def algo_select(algo): # this just for saving the selected algo
    ...
    if algo == "bfs":
        ...
        document["paragh"].innerText = "Selected Branch and bound + estimated jus"
    elif algo == "astar":
...
def ads_select():
    # Additional imporvments.
    buttons =['bfs','dfs','hc','bs','best','bb','bb-h','bb-ex','astar']
    if(document['ads'].checked):
        document['setstart'].innerText='Set Max'
        document['setgoal'].innerText='Set Min'
        document['na-1'].style.display='block'
        document['na-2'].style.display='block'
        document['ads_nt1'].style.display='none'
        document['ads_nt2'].style.display='none'
        document['ads_t'].style.display = 'block'
        for i in buttons :
            document[i].disabled=True 
        pass
    else:
        document['setstart'].innerText='Set Start'
        document['setgoal'].innerText='Set End'
        document['na-1'].style.display='none'
        document['na-2'].style.display='none'
        document['ads_nt1'].style.display='block'
        document['ads_nt2'].style.display='block'
        document['ads_t'].style.display = 'none'
        for i in buttons :
            document[i].disabled=False 
        pass
...
if __name__=='__main__':
    ...
    colors_for_ads = {'Min' : 'pink','Max':'purple'}
    ...
    map_algorithm = {'bfs' : agent.bfs, 'dfs' : agent.dfs , 'hc' : agent.hc \
        ,'bs' : agent.bs,'best': agent.best ,'bb' : agent.bb, 'bb-h' : agent.bb_h,'bb-ex':agent.bb_ex,'astar' : agent.a_star }
    # map_algorithm = {'bfs' : agent.bfs, 'dfs' : agent.dfs , 'hc' : agent.hc \
    ...
    # For choosing ADG vs Searches
    document['ads'].bind('change',lambda e:ads_select())
    ...
    document['bb-ex'].bind('click', lambda e: algo_select('bb-ex'))
